...
//------------------------------------------------------------------------------
template<> uint8_t*
VertexCodec::Encode<VertexFormat::Float>(uint8_t* dst, const glm::vec4& scale, const float* src, int numSrcComps) {
    float* p = (float*) dst;
    *p++ = *src * scale.x;
    return (uint8_t*)p;
}
...
//------------------------------------------------------------------------------
template<> uint8_t*
VertexCodec::Encode<VertexFormat::Float2>(uint8_t* dst, const glm::vec4& scale, const float* src, int numSrcComps) {
    float* p = (float*) dst;
    for (int i = 0; i < 2; i++) {
        *p++ = (numSrcComps > i) ? src[i] * scale[i] : 0.0f;
    }
    return (uint8_t*)p;
}
...
//------------------------------------------------------------------------------
template<> uint8_t*
VertexCodec::Encode<VertexFormat::Float3>(uint8_t* dst, const glm::vec4& scale, const float* src, int numSrcComps) {
    float* p = (float*) dst;
    for (int i = 0; i < 3; i++) {
        *p++ = (numSrcComps > i) ? src[i] * scale[i] : 0.0f;
    }
...
//------------------------------------------------------------------------------
template<> uint8_t*
VertexCodec::Encode<VertexFormat::Float4>(uint8_t* dst, const glm::vec4& scale, const float* src, int numSrcComps) {
    float* p = (float*) dst;
    for (int i = 0; i < 4; i++) {
        *p++ = (numSrcComps > i) ? src[i] * scale[i] : 0.0f;
    }
...
//------------------------------------------------------------------------------
template<> uint8_t*
VertexCodec::Encode<VertexFormat::Byte4>(uint8_t* dst, const glm::vec4& scale, const float* src, int numSrcComps) {
    const float x = src[0] * scale.x;
    const float y = (numSrcComps > 1) ? src[1] * scale.y : 0.0f;
    const float z = (numSrcComps > 2) ? src[2] * scale.z : 0.0f;
    const float w = (numSrcComps > 3) ? src[3] * scale.w : 0.0f;
...
//------------------------------------------------------------------------------
template<> uint8_t*
VertexCodec::Encode<VertexFormat::Byte4N>(uint8_t* dst, const glm::vec4& scale, const float* src, int numSrcComps) {
    glm::vec4 s = scale * 127.0f;
    const float x = src[0] * s.x;
    const float y = (numSrcComps > 1) ? src[1] * s.y : 0.0f;
    const float z = (numSrcComps > 2) ? src[2] * s.z : 0.0f;
    const float w = (numSrcComps > 3) ? src[3] * s.w : 0.0f;
...
//------------------------------------------------------------------------------
template<> uint8_t*
VertexCodec::Encode<VertexFormat::UByte4>(uint8_t* dst, const glm::vec4& scale, const float* src, int numSrcComps) {
    const float x = src[0] * scale.x;
    const float y = (numSrcComps > 1) ? src[1] * scale.y : 0.0f;
    const float z = (numSrcComps > 2) ? src[2] * scale.z : 0.0f;
    const float w = (numSrcComps > 3) ? src[3] * scale.w : 0.0f;
...
//------------------------------------------------------------------------------
template<> uint8_t*
VertexCodec::Encode<VertexFormat::UByte4N>(uint8_t* dst, const glm::vec4& scale, const float* src, int numSrcComps) {
    glm::vec4 s = scale * 255.0f;
    const float x = src[0] * s.x;
    const float y = (numSrcComps > 1) ? src[1] * s.y : 0.0f;
    const float z = (numSrcComps > 2) ? src[2] * s.z : 0.0f;
    const float w = (numSrcComps > 3) ? src[3] * s.w : 0.0f;
...
//------------------------------------------------------------------------------
template<> uint8_t*
VertexCodec::Encode<VertexFormat::Short2>(uint8_t* dst, const glm::vec4& scale, const float* src, int numSrcComps) {
    const float x = src[0] * scale.x;
    const float y = (numSrcComps > 1) ? src[1] * scale.y : 0.0f;
...
//------------------------------------------------------------------------------
template<> uint8_t*
VertexCodec::Encode<VertexFormat::Short2N>(uint8_t* dst, const glm::vec4& scale, const float* src, int numSrcComps) {
    glm::vec4 s = scale * 32767.0f;
    const float x = src[0] * s.x;
    const float y = (numSrcComps > 1) ? src[1] * s.y : 0.0f;
...
//------------------------------------------------------------------------------
template<> uint8_t*
VertexCodec::Encode<VertexFormat::Short4>(uint8_t* dst, const glm::vec4& scale, const float* src, int numSrcComps) {
    const float x = src[0] * scale.x;
    const float y = (numSrcComps > 1) ? src[1] * scale.y : 0.0f;
    const float z = (numSrcComps > 2) ? src[2] * scale.z : 0.0f;
    const float w = (numSrcComps > 3) ? src[3] * scale.w : 0.0f;
...
//------------------------------------------------------------------------------
template<> uint8_t*
VertexCodec::Encode<VertexFormat::Short4N>(uint8_t* dst, const glm::vec4& scale, const float* src, int numSrcComps) {
    glm::vec4 s = scale * 32767.0f;
    const float x = src[0] * s.x;
    const float y = (numSrcComps > 1) ? src[1] * s.y : 0.0f;
    const float z = (numSrcComps > 2) ? src[2] * s.z : 0.0f;
    const float w = (numSrcComps > 3) ? src[3] * s.w : s.w;    // FIXME? special case for positions
